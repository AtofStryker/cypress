+describe('multi-domain Cypress API', { experimentalSessionSupport: true, experimentalMultiDomain: true }, () => {
  ...
  context('methods', () => {
    ...
    // verifies the config from the primary domain is synced with the secondary
    it('config()', () => {
    ...
    /**
     * Cypress.env() is not synced intentionally. Instead, maybe encourage users to pass in env variables via args to switchToDomain for security reasons?
     * it('shows users have to interact with env() cross-domain', () => {
     *    Cypress.env('username', 'username')
     *    Cypress.env('password', '*****')
     *
     *     cy.switchToDomain('foobar.com', [Cypress.env('username'), Cypress.env('password')], ([username, password]) => undefined)
     * })
     */
    ...
    it('env()', () => {
      ...
      cy.switchToDomain('foobar.com', [env], ([theEnv]) => {
        ...
        expect(multiDomainEnv).to.not.deep.equal(theEnv)
      })
    })
  })
